(function() {
  // 1. The ONLY link that matters
  var github = "https://cdn.jsdelivr.net/gh/goat-67/ugs-singlefile@main/UGS-Files/assets/";

  // Helper to fix any URL
  var fix = function(u) {
    if (typeof u !== 'string' || u.includes('jsdelivr') || u.includes('data:')) return u;
    var name = u.split('/').pop();
    // If it looks like a file, force it to GitHub
    if (name.includes('.')) {
      console.log("%c🌟 Force Loading: " + name, "color: yellow; background: black; font-weight: bold;");
      return github + name;
    }
    return u;
  };

  // 2. Hijack "Image" loading
  ...
  // 3. Hijack "Fetch" (This is usually why games stay blank!)
  var oF = window.fetch;
  window.fetch = function(i, s) {
    var u = (typeof i === "string") ? i : (i.url || "");
    var nU = fix(u);
    if (nU !== u) i = (typeof i === "string") ? nU : new Request(nU, i);
    return oF(i, s);
  };
})();
